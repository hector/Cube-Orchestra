/* 
slicerec~ version 0.9.11, an object class for Pure Data.
...
version 0.9.1: used garray_getfloatwords() instead of garray_getfloatarray(), for 64 bit compatibility
*/
...
{
	t_float cuepoint = (t_float)x->cuepoint;
	
	if(x->recstatus) outlet_float(x->attackoutlet, cuepoint);
	else outlet_float(x->silenceoutlet, cuepoint);
...
static void slicerec_inserttwoblocks(t_slicerec *x)
{
	...
	{
		window = (0.5 * cos(angle * n)) + 0.5;						// smooth fade-in
...
{
	t_int n, timeindex;
	t_int windowsize = COSTABSIZE>>1;							// Pd API cos_table[] is used for fade out
	t_int mask = x->loopsize - 1;
	t_word *arraypointer = x->arraypointer;
	
	timeindex = (x->timeindex + x->loopsize - windowsize) & mask;		// find start of fade out region
	
	for(n=x->blocksize; n; n--, timeindex++)
	{
		arraypointer[timeindex].w_float *= 0.5 - (0.5 * cos_table[n]);		// smooth out discontinuities
		timeindex &= mask;
	}
}
...
static t_int *slicerec_perform(t_int *w)
{
	...
	if(!timeindex && recstatus) 
		arraypointer[x->loopsize].w_float = *input;				// x[0] = x[N ] for linear interpolated reading
...
void slicerec_tilde_setup(void)
{
	...
	post("slicerec~ version 0.9.11 written by Katja Vetter");
}
