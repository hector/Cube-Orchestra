...
public class EffectDragAction implements IGestureEventListener {
	private IMTComponent3D dragTarget;
	private boolean useCustomTarget;
	
	public EffectDragAction(){
		this.useCustomTarget = false;
	}
	
	public EffectDragAction(IMTComponent3D dragTarget){
	...
	public boolean processGestureEvent(MTGestureEvent g) {
		if (g instanceof DragEvent){
			...
			case MTGestureEvent.GESTURE_DETECTED:
				...
				checkEnabledInstruments(dragTarget);
				break;
	...
	private void checkEnabledInstruments(IMTComponent3D dragTarget) {
		if (dragTarget instanceof Effect) {
			Effect effect = (Effect) dragTarget;
